...
import { pipe, switchMap, tap, catchError, of, concatMap, EMPTY } from 'rxjs';
...
export const CourseStore = signalStore(
  ...
  withComputed((store) => ({
    ...
    fullCourses: computed(() => store.courses().filter((c) => c.enrollmentCount >= c.maxCapacity)),
  ...
  withMethods((store, svc = inject(CourseService)) => ({
      ...
      pipe(
        ...
        switchMap(({ id, payload }) =>
          svc.update(id, payload).pipe(
            tap(() => {
              // Replace in the list
              const courses = store.courses().map((c) => (c.id === id ? { ...c, ...payload } : c));

              // Also update detail if it is currently open
              const selected = store.selectedCourse();
              const selectedCourse = selected?.id === id ? { ...selected, ...payload } : selected;
    ...
    deleteCourse(id: number) {
      ...
      const previousSnapshot = store.courses();
      // 2. Instant visual feedback — remove entity immediately fromlocal UI
      patchState(store,{
        courses: previousSnapshot.filter(c=>c.id !==id),
        error: null,
      });
